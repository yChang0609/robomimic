...
from robomimic.algo import ResidualRolloutPolicy, RolloutPolicy
...
def inference(args):
    ...
    for i, ckpt_path in enumerate(args.agents):
        ...
        if ckpt_dict["algo_name"] == "residual_policy":
    ...
    for key in pbar:
        ...
        pbar.set_postfix(
            Success=f"{int(success_count)}/{len(results)}",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Test a trained policy on failed MimicGen environments."
    )
    ...
    parser.add_argument(
        "--video_path", type=str, default=None, help="Path to save output video"
    )
    parser.add_argument(
