...
You can use the `Dockerfile` to easily build a containerized environment for setting up robomimic with Python 3.9, Miniconda, robosuite, and PyTorch (40-series / 50-series GPU variants).

### Build with Makefile (recommended)

Build image (40-series GPUs, no optional envs):
```bash
make build GPU_SERIES=40 INSTALL_OPTIONAL=0
```

Build image (50-series GPUs, install optional envs like MimicGen):
```bash
make build GPU_SERIES=50 INSTALL_OPTIONAL=1
```

Image tag is auto-generated by Makefile:
`robomimic:gpu<GPU_SERIES>-opt<INSTALL_OPTIONAL>`

Key variables:
- `GPU_SERIES`: `40` or `50`
- `INSTALL_OPTIONAL`: `0` (skip optional envs) or `1` (install optional envs)

Run container (interactive shell):
```bash
make run GPU_SERIES=50 INSTALL_OPTIONAL=1 DATASETS_DIR=$HOME/datasets
```

Run container and keep it alive:
```bash
make run-tail GPU_SERIES=50 INSTALL_OPTIONAL=1 DATASETS_DIR=$HOME/datasets
```

Extra run variables:
- `DATASETS_DIR`: host path mounted to `/datasets` in container
- `CONTAINER_NAME`: docker container name (default: `robomimic`)
- `RUN_IMAGE`: override image name:tag to run

### Build / Run with raw docker commands
